...
def create_monetary_policy_stack(df):
    ...
    # Top panel: Stock Variables (Stacked Areas)
    ax1 = axes[0]
    
    # Normalize for stacking (convert to trillions for readability)
    m2_norm = df['M2SL'] / 1000 if 'M2SL' in df.columns else pd.Series(index=df.index)
    walcl_norm = df['WALCL'] / 1000000 if 'WALCL' in df.columns else pd.Series(index=df.index)  # WALCL is in millions
    rrr_norm = df['RRPONTSYD'] / 1000 if 'RRPONTSYD' in df.columns else pd.Series(index=df.index)
    
    # Create stacked areas for monetary policy tools
    ...
    if not rrr_norm.empty and rrr_norm.max() > 0:
        # Scale RRR for visibility (multiply by 100 to show alongside trillions)
    ...
    # Bottom panel: Flow Variables (Rate Environment)
    ax2 = axes[1]
    ...
    # Add VIX on secondary axis for context
...
def create_asset_cumulative_context(df):
    ...
    # 1. Bitcoin - Revolutionary Digital Asset
    ax1 = axes[0, 0]
    ...
    # 2. Gold - Traditional Store of Value  
    ax2 = axes[0, 1]
    ...
    # 3. NASDAQ 100 - Innovation Economy
    ax3 = axes[1, 0]
    ...
    # 4. Dollar Index - Global Reserve Currency
    ax4 = axes[1, 1]  
    if 'DXY' in df.columns:
        dxy_data = df['DXY'].dropna()
        # Center around 100 for DXY context
        baseline = 100
...
def create_economic_regime_analysis(df):
    ...
    # Panel 1: Monetary Policy Stance
    ax1 = axes[0]
    
    # M2 growth rate (annualized)
    if 'M2SL' in df.columns:
        m2_growth = df['M2SL'].pct_change(periods=252).dropna() * 100  # Annualized
        ax1.plot(m2_growth.index, m2_growth.values, linewidth=3, color='steelblue',
                label='M2 Growth Rate (Annualized %)')
    
    # Fed funds rate proxy (TNX)  
    if 'TNX' in df.columns:
        ax1_twin = ax1.twinx()
    ...
    # Panel 2: Risk Asset Performance  
    ax2 = axes[1]
    
    # Normalize all assets to start at 100 for relative comparison
    ...
    # Panel 3: Market Stress & Volatility
    ax3 = axes[2]
    ...
    if 'VIX' in df.columns and 'BAMLH0A0HYM2' in df.columns:
        ...
        # Credit spreads on secondary axis
        ax3_twin = ax3.twinx()
...
def create_flow_stock_framework(df):
    ...
    # Stock Variables (Levels - use area charts)
    stock_vars = [
    ...
    # Flow Variables (Rates - use line charts)  
    flow_vars = [
...
def generate_structural_insights(df):
    ...
    # Correlation Insights by Asset Class
    print("\\n🔗 STRUCTURAL RELATIONSHIPS:")
    
    # Monetary policy correlation
