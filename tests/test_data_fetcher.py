...
class TestDataFetcher:
    ...
    def test_fetch_fred_history_success(self, mock_get):
        ...
        result = self.fetcher.fetch_fred_history("M2SL", date(2023, 1, 1))
    ...
    def test_fetch_yahoo_current_fallback(self, mock_requests, mock_ticker_class):
        ...
        mock_response.json.return_value = {
            "quoteResponse": {"result": [{"regularMarketPrice": 110.0}]}
        }
        mock_response.raise_for_status.return_value = None
    ...
    def test_fetch_all_current(self, mock_yahoo, mock_fred):
        ...
        for ticker_id in config.yahoo_tickers:
            assert ticker_id in result

    def test_fetch_all_current_with_fallback(self):
        """Test fetch with fallback data."""
        fallback_data = {"M2SL": 19500.0, "BTCUSD": 45000.0}
        with (
            patch.object(self.fetcher, "fetch_fred_current", return_value=None),
    ...
    def test_to_date_index(self):
        ...
        df = pd.DataFrame(
            {"DATE": ["2023-01-01", "2023-01-02"], "VALUE": [100, 110]}
        )
...
class TestDataIntegration:
    ...
    def test_indicators_by_category(self):
        ...
        assert all("source" in item for item in monetary_policy.values())
